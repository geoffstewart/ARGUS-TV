...
namespace ArgusTV.GuideImporter.JSONService
{
	...
	public class WebClient
	{
		...
		const string baseUrl = baseUrlWithoutVersion + "/20140530/";
...
//			wr.KeepAlive = false; // otherwise, there are intermittent errors with connections closing
		...
		/// <returns></returns>
		public Dictionary<string, HeadendResponseInstance> getHeadends(TokenRequest tr, string country, string postalcode) {
			Dictionary<string, HeadendResponseInstance> retHashMap = new Dictionary<string, HeadendResponseInstance>();
			
			string token = getToken(tr);

			string url = baseUrl + "headends?country=" + country + "&postalcode=" + postalcode;
			
			retHashMap = executeHttpRequest<Dictionary<string, HeadendResponseInstance>>(token, "GET", url);
		...
		public List<HeadendResponseInstance.Lineup> getLineupsFromHeadends(TokenRequest tr, string country, string postalcode) 
		{
			...
			Dictionary<string, HeadendResponseInstance> dic = getHeadends(tr, country, postalcode);
			
			foreach(var he in dic.Values) {
		...
		public List<SchedulesResponse> getSchedules(TokenRequest tr, List<string> stationIdList, int days) {
			...
			lsr = executeHttpRequestWithList<SchedulesResponse>(token, "POST", baseUrl + "schedules", req);
		...
		public List<ProgramResponseInstance> getPrograms(TokenRequest tr, ICollection<string> programIds) {
			...
			lpri = executeHttpRequestWithList<ProgramResponseInstance>(token, "POST", baseUrl + "programs", programIds);
